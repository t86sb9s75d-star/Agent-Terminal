...
- **Real cost tracking** — token usage is read from each provider's response and priced against a documented table (`src/pricing.js`). The model a run is priced against is the one it actually executed against: an agent left without an explicit model resolves to its provider's default once, before the run starts, and that same value is what gets invoked, recorded on the run, written to the audit event, and priced (`src/models.js`) — so default-model usage can't be real spend that reports as unpriced. Cost aggregates are structured (`complete` / `partial` / `unavailable` / `empty`) so a mix of priced and unpriced runs is never silently summed and shown as if it were a complete total — see `test/runsStore.pricing.test.js` and `test/effectiveModel.test.js`.
...
- `docs/FEATURE_ONBOARD.md` — the workspace operating layer: architecture, progress formulas, and an explicit recorded-vs-enforced permission table
- `docs/ENGINEERING_FINDINGS.md` — every defect found while building and reviewing this, with reproduction, root cause, and the mutation each regression test was proven against
- `docs/PRODUCT_INSIGHTS.md` — generalized engineering patterns drawn from those defects, labeled as unvalidated hypotheses
